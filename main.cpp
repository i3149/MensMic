...
int run_keyboard(AudioUnit* synthUnit, input_buffer buffer, UInt8 midiChannel) {
    ...
    //nextNote = (rand() % 100 > 15)? notes[data[TG_DATA_ATTENTION] % 7]: minor[data[TG_DATA_ATTENTION] % 5];
  	//nextDuration = g_durations[data[TG_DATA_DELTA] % g_num_durations];
    
    UInt32              notes[] = {60,62,66,64,66,67,69,71};
    UInt32              octives[] = {-8,0,8};
    UInt32              nextNote;
    UInt32              nextDuration;
		UInt32              onVelocity;
    UInt32              lastNote = 67;
    ...
    while (true) {
        ...
        if (data[TG_END_NOTE]) {
            ...
            require_noerr (result = MusicDeviceMIDIEvent(*synthUnit, noteOnCommand, notes[5], 100, 0), finished);
            usleep (nextDuration);      
            require_noerr (result = MusicDeviceMIDIEvent(*synthUnit, noteOnCommand, notes[5], 0, 0), finished);
            return 1;  
        }
        ...
        if (nextNote - lastNote > 4 && rand() % 10 > 7) {
            nextNote = lastNote + 1;
        } else if (lastNote - nextNote > 4 && rand() % 10 > 7) {
            nextNote = lastNote - 1;
        }
...
int main (int argc, const char * argv[]) {
    ...
    //    while(numPackets<100) {    
        ...
        /** For testing
         ...
         dataHolder[TG_DATA_THETA] = rand()%80000;
        ...
        if(numPackets > 0){
            ...
            if (signalQuality < 52) {
                ...
                fprintf(stdout, "Sig: %3.0f, Att: %3d, Med: %3d, D: %3d, T: %3d, Blink: %3.0f\n", 
                        signalQuality, 
                        dataHolder[TG_DATA_ATTENTION], 
                        dataHolder[TG_DATA_MEDITATION], 
                        dataHolder[TG_DATA_DELTA], 
                        dataHolder[TG_DATA_THETA],
                        blink);
                fflush(stdout);
                
                
                if (dataHolder[TG_DATA_ATTENTION] > 0) { 
